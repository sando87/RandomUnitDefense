...
public class MultiMissile : SkillBase
{
    ...
    GameObject DoRandomShot()
    {
        ...
        firePosition += new Vector3(randomOffX, randomOffY, 0);
        // 투사체 객체 생성
        ...
        Vector3 dest = firePosition + (Direction * ProjectileMaxDistance) - Vector3.up;
        StartCoroutine(MoveProjectile(projectileObj, dest, ProjectileDuration, () =>
        {
    ...
    IEnumerator MoveProjectile(GameObject proj, Vector3 dest, float duration, System.Action eventEnd)
    {
        ...
        while(time < duration)
        {
            ...
            float rad = offRad + time * 10;
            float offY = Mathf.Sin(rad) * 0.2f;
            curPos.y += offY;
            proj.transform.position = curPos;
            proj.transform.right = (curPos - prePos).normalized;
            prePos = curPos;
            yield return null;
            if(proj == null)
                yield break;
